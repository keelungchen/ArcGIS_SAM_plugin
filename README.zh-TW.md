...
- **屬性標籤下拉選單** — 指定目標圖層的某個欄位與它的 **domain 值**
  （或子類型），存下來的每個 polygon 就自動帶上該值 —— 邊圈邊貼標籤，
  不必事後開屬性表逐一填。沒有 domain 的欄位會標示出來並可手動輸入。
- **凍結工作區** — 第一次點擊時擷取範圍並快取其 embedding，之後的點擊
  幾乎是即時反應。
- **地圖疊加面板** — 顯示目前模型、目前標籤、點擊數量、信心分數與狀態，
  並有儲存／清除點擊／重設工作區按鈕。
...
3. （可選）在 **Attribute Label（屬性標籤）** 群組挑一個
   **Label field（標籤欄位）**，再挑一個 **Label（標籤值）** —
   之後每個存下來的 polygon 都會自動帶上這個值。詳見 8.1。
4. 點擊 **Start Server（啟動伺服器）**（arcpy 與模型預載時會出現進度
   對話框）。等待狀態轉為綠色。
5. 點擊 **Segment** 工具，然後在地圖上點擊：
...
6. **工作區**會在你第一次點擊時從當時的地圖檢視凍結。使用功能區的
   **New Work Area** / **Cancel Work Area** 按鈕來移動或取消它。
7. 地圖上的疊加面板會顯示模型、目前標籤、點擊數量、信心分數與狀態，
   並有儲存／清除點擊／重設工作區按鈕。
...
- 對於珊瑚／底棲影像，**RITM** 引擎的開箱表現常常勝過 SAM，而且在
  CPU 上就跑得很順。
- 建議**一次做一個類別**：先設定好 **Label**，把該類別的物件全部圈完
  再換標籤 — 比事後開屬性表逐一填值快得多。

### 8.1 邊圈邊標籤（Attribute Label）

功能區的 **Attribute Label（屬性標籤）** 群組會在建立 polygon 的同時
就把屬性值寫進去，不必在物件之間切換到「屬性」窗格：

| 下拉選單 | 作用 |
|---|---|
| **Label field（標籤欄位）** | 要寫入標籤的**目標圖層欄位**。每次展開選單都會重新讀取目前目標圖層的欄位 |
| **Label（標籤值）** | 之後每個存檔的 polygon 要寫入的值。隨時可以切換，下一個 polygon 就換成新標籤 |

欄位選單會標示每個欄位的值從哪裡來：

| 標示 | 意義 | 怎麼選值 |
|---|---|---|
| `[domain: n values]` | 欄位有**編碼值域（coded value domain）** | 在 **Label** 選單挑描述，實際寫入的是**代碼（code）** |
| `[subtypes: n]` | 這是圖層的**子類型（subtype）欄位** | 挑子類型名稱，寫入其代碼 |
| `[range domain]` | 範圍值域（只有最小／最大值） | 直接在 **Label** 欄位輸入值後按 Enter |
| `[no domain]` | **沒有值域** | 直接在 **Label** 欄位輸入值後按 Enter |

**選到沒有 domain 的欄位時會跳出提示**（右上角鈴鐺通知）告訴你這個欄位
沒有值域；若想要有固定清單可選，請先在地理資料庫替該欄位掛上編碼值域
（*Fields 檢視 → Domain*）。

選 `(no value)` 可停止寫入標籤；在欄位選單選 `(no label)` 則完全關閉標籤
功能。**更換 Target 圖層時兩個選單都會清空**（欄位與值域屬於該圖層）。
目前的標籤會以綠色顯示在地圖疊加面板上，每次存檔的通知也會標明，例如
`Polygon saved and selected (score 0.94, Class = Acropora).`
...
| SAM 頁籤在，但**所有控制項都是灰的／下拉打不開** | **在 Pro 開著的時候重新安裝增益集**，Pro 的 `AssemblyCache` 留著被鎖住的舊 DLL，與新的 `Config.daml` 不一致，組件載入失敗 | 關閉 Pro，執行 `Remove-Item "$env:LOCALAPPDATA\ESRI\ArcGISPro\AssemblyCache\{8f3b0f0a-9c1e-4f5d-b1a2-6d9d1e7c4a55}" -Recurse -Force` 後再開 Pro。2.6.1 起 `build_addin.ps1 -Install` 會自動清除此快取，並在 Pro 還開著時警告 |
| **「The requested extent does not overlap the input raster」** | **Imagery 選到的不是目前檢視範圍下的那張影像**（一個專案有多個場址時很容易發生） | 展開 **Imagery** 下拉，選標示 `[in view]` 的圖層；2.6.1 起增益集會直接在訊息中告訴你該選哪一張，不會再丟到伺服器才失敗 |
...
另外提供**純 RITM 安裝版**（`INSTALL_RITM_ONLY.bat`），略過 SAM 的
python 套件與 SAM 地理處理工具箱。

### 2.6.1 — 2026-08-02
修正多場址專案的影像選取問題。增益集現在會**在送出伺服器之前**先確認地圖
檢視範圍與所選 **Imagery** 圖層有交集，並直接指出畫面實際位於哪一張影像
之上，而不是丟出伺服器端的
`The requested extent does not overlap the input raster`。下拉選單會把涵蓋
目前畫面的影像標示為 `[in view]`；若選過的影像在目前地圖中找不到（在別的
地圖選的、或圖層已被移除），不再默默改用地圖中的「第一張」影像，而是採用
畫面下方的那張。**Imagery** / **Target** 下拉在清單刷新後也會保持顯示已選
取的圖層。

### 2.6.0 — 2026-08-02
增益集新增 **Attribute Label（屬性標籤）** 功能區群組：指定目標面圖層的
**Label field（標籤欄位）** 與 **Label（標籤值）** 後，按 `Space` 存下的
每個 polygon 都會直接帶上該屬性值。值來自欄位的編碼值域（coded value
domain）或圖層子類型；欄位清單會標示 `[domain: n values]`／
`[subtypes: n]`／`[range domain]`／`[no domain]`，**選到沒有 domain 的
欄位會跳出提示**並改用手動輸入（依欄位型別驗證）。目前標籤顯示在地圖
面板上，切換目標圖層時自動清空。詳見 [8.1](#81-邊圈邊標籤attribute-label)。
